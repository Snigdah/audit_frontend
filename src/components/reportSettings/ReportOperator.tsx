...
import ReportOperatorAddModal from "./ReportOperatorAddModal";
...
const ReportOperator = ({ reportId }: ReportOperatorProps) => {
  ...
  const fetchOperators = (
    ...
    setLoading(true);
    ReportService.fetchOperatorsByReport(Number(reportId), {
      search,
      page,
      size,
      all: false,
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      await ReportService.removeOperator(Number(reportId), selectedOperatorId);
  ...
  return (
    <div className="p-4 bg-white rounded-lg shadow-sm">
        ...
        <Table
          ...
          locale={{
            emptyText: searchText
              ...
              : "No operators assigned to this report",
          }}
          onRow={(record) => ({
      ...
      <DeleteConfirmationModal
        ...
        description="Are you sure you want to remove this operator from the report?"
        confirmText="Remove"
      />
    </div>
  );
