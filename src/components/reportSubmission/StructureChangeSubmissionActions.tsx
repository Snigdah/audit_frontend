...
interface StructureChangeSubmissionActionsProps {
  reportId: number;
  submissionId: number;
...
const StructureChangeSubmissionActions = ({
  reportId,
  submissionId,
  status,
  onActionComplete,
}: StructureChangeSubmissionActionsProps) => {
  ...
  const isAdmin = Boolean(
    authState.role && authState.role.toLowerCase() === "admin"
  );
  ...
  const handleFormSubmit: SubmitHandler<ReviewFormData> = async (data) => {
    ...
    try {
      ...
      await ReportSubmissionService.approveSupervisorStructureChange(
        reportId,
        submissionId,
        payload
      );
      ...
      toast.success(
        reviewAction === "APPROVED"
          ? "Structure change request approved successfully"
          : "Structure change request rejected"
      );
  ...
  return (
    <>
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-4">
          <div>
            <h3 className="text-base font-semibold text-gray-800 mb-1">
              Review Structure Change Request
            </h3>
            <p className="text-sm text-gray-600">
              Approve or reject this structure change request
            </p>
          </div>
          <Space size="small">
            <Button
      ...
      <ModalComponent
        title={
          reviewAction === "APPROVED"
            ? "Approve Structure Change Request"
            : "Reject Structure Change Request"
        }
...
export default StructureChangeSubmissionActions;
