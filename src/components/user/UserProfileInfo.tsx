...
import type { ProfileBasic } from "../../types/profile";

type UserProfileInfoProps = {
  employeeId: string; // pass this from admin view
};

const UserProfileInfo = ({ employeeId }: UserProfileInfoProps) => {
  ...
  useEffect(() => {
    if (employeeId) fetchUserProfile();
  }, [employeeId]);

  const handleChangePassword = () => setChangePasswordModalVisible(true);
  const handleChangePasswordCancel = () => setChangePasswordModalVisible(false);
  ...
  if (!profile)
    return (
      <div className="flex items-center justify-center p-12">
        <p className="text-gray-500">No user profile information found.</p>
      </div>
    );
  ...
  return (
    <div className="p-4 bg-white rounded-lg shadow-sm">
      ...
      <ChangePasswordModal
        ...
        employeeId={employeeId} // pass for admin mode
      />
    </div>
  );
};
